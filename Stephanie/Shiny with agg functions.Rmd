...
# Shiny output 2: So where is the project grant money coming from for each county?
renderPlot({
...
#?    agg.per.l <- filter(agg.per.l, fund > 0) -- what if var is empty?
...
radioButtons( inputId = 'recipient', 
              ...
              choices = c("All", sort(unique(gra16.3$recip_cat_type))), 
              selected = "All")

#Shiny output 3: Who is getting what types of project grants?

renderPrint({

if (input$recipient != "All"){
    
gra16.4 <- filter(gra16.3 , county %in% input$your_county , assistance_type == "04: Project grant", recip_cat_type == input$recipient) 

agg.per <- agg.county(gra16.4, gra16.4$cfda_program_title)

} else if ( input$recipient == "All" ){
