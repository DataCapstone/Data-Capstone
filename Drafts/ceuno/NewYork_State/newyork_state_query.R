...
### Import remaining geocode
other_geocode_url <- "https://github.com/DataCapstone/Data-Capstone/blob/master/Drafts/ceuno/NewYork_State/ny_geocode_partial2.rds?raw=true"
geocode_results2 <- readRDS( gzcon( url( other_geocode_url ) ) )
# bind results
all_geocode <- rbind.data.frame(geocode_results, geocode_results2)
# check dimensions
dim( all_geocode )
library(dplyr)
# Merge!
newyork_state_geocoded <- left_join( x = all_geocode
                            , y = newyork_state
                            , by = "recipient.full.address"
)
### Save as RDS
saveRDS( newyork_state_geocoded
         , file = "/Users/cristiannuno/Desktop/Syracuse/MPA_Portfolio/DATA_ACT/data_export/fy17_ny_awards_geocoded.rds"
         )
# How many NA, NA, NYs do we have?
sum( 
  stri_count(newyork_state_geocoded$recipient.full.address
             , fixed = "NA, NA, NY"
             )
) # 4981
